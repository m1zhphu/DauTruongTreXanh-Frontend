...
import { Plus, Trash, Image as ImageIcon, Save, ArrowLeft, Clock, Type, FileText, Upload, Loader2, CheckCircle } from "lucide-react";
...
const EditQuiz = () => {
  ...
  useEffect(() => {
      const fetchQuizData = async () => {
          try {
              const res = await httpAxios.get(`/live-quiz/${id}`);
              const data = res.data;
              
              // Đảm bảo questions luôn là mảng (tránh lỗi null)
              if (!data.questions) data.questions = [];
              
              setQuizData(data);
          } catch (err) {
              console.error("Lỗi tải dữ liệu:", err);
              alert("Không thể tải bài Quiz này (Lỗi 500 hoặc không tồn tại).");
              navigate("/my-quizzes");
          } finally {
              setFetching(false);
          }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await httpAxios.put(`/live-quiz/${id}`, quizData);
          ...
          <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
              <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-amber-500 rounded-lg flex items-center justify-center text-white font-bold">E</div>
                      <span className="font-bold text-slate-700 hidden sm:block">Chỉnh sửa Quiz</span>
                  </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="bg-amber-500 hover:bg-amber-600 text-white px-6 py-2 rounded-lg font-bold text-sm shadow-sm transition-all flex items-center gap-2 active:scale-95 disabled:opacity-70"
                  >
      ...
      <div className="max-w-[1400px] mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-9 space-y-6">
                ...
                {quizData.questions.map((q, qIdx) => (
                    <div key={qIdx} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden group hover:border-slate-300 transition-all">
                        ...
                        <div className="bg-white px-5 py-3 border-b border-slate-100 flex justify-between items-center">
                            ...
                            <button onClick={() => handleRemoveQuestion(qIdx)} className="text-slate-400 hover:text-red-500 hover:bg-red-50 p-1.5 rounded transition-colors">
                                <Trash size={16} />
                            </button>
                        </div>
                        ...
                        <div className="p-6">
                            <div className="flex flex-col md:flex-row gap-6 mb-6">
                                ...
                                >
                                    ...
                                    <input type="file" className="hidden" accept="image/*" ref={(el) => (questionImageRefs.current[qIdx] = el)} onChange={(e) => handleImageUpload(e, 'question', qIdx)} disabled={uploading} />
                                </div>
                            </div>
                                    ...
                                    return (
                                        <div key={oIdx} className="flex items-center gap-0">
                                            ...
                                            <div className="flex-1 relative">
                                                ...
                                                <input 
                                                    className={`w-full pl-16 pr-10 py-3 border rounded-r-md outline-none transition-all text-sm font-medium ${isCorrect ? `bg-green-50 border-green-500 text-green-800` : `bg-white border-slate-200 focus:border-slate-400`}`}
                                                    value={opt}
                                                    onChange={(e) => updateOption(qIdx, oIdx, e.target.value)}
                                                />
                                                <button onClick={() => setCorrect(qIdx, opt)} className={`absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-full transition-all ${isCorrect ? "text-green-600 bg-white shadow-sm" : "text-slate-300 hover:text-slate-500"}`}>
                ...
                <button onClick={handleAddQuestion} className="w-full py-4 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 hover:border-slate-400 hover:text-slate-700 hover:bg-white transition-all flex flex-col items-center justify-center gap-1 group bg-slate-50/50">
